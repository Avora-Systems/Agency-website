...
- `js/script.js` — header scroll state (`#site-header` → `.is-scrolled`), hamburger overlay menu (`#menu-toggle` / `#nav-overlay`, body scroll-lock, Esc to close), GSAP `ScrollTrigger.batch` scroll-reveal for `.reveal` elements (fade + rise with stagger; falls back to adding `.is-visible` to all `.reveal` when GSAP is unavailable or reduced motion is requested), a guarded testimonials slider (no-ops unless `#testimonials` exists), contact form submission. All respect `prefers-reduced-motion`.
